...
A $Q$ plane object is implemented as a C++ class inheriting from the 2-D histogram class of ROOT (\texttt{TH2D}). The number of $Q$ planes, $N_Q$, is equal to the next integer verifying the condition~\ref{eq:tiledistanceq} and the $Q$ value associated to the plane indexed by $q$ is
\begin{equation}
  Q_q = Q_{min}\left[ \frac{Q_{max}}{Q_{min}}\right]^{(0.5+q)/N_q}, \qquad 0\le q < N_Q.
\end{equation}
The $Q$ plane histogram is first binned into logarithmically-spaced frequency rows. The number of frequency rows, $N_\phi$, is equal to the next integer verifying the condition~\ref{eq:tiledistancephi}. The time axis is divided into $N_\tau$ linear bins. $N_\tau$, is taken to be the next power of two verifying the condition~\ref{eq:tiledistancetau}. As we will see in Sec.~\ref{sec:algorithm:projection}, working with a power-of-two number of tiles is computationally more efficient. The tile time and frequency, $(\tau_m, \phi_l)$, is given by the histogram bin center:
\begin{align}
...
\end{align}
In addition, the tile is also given a duration and a bandwidth corresponding to the bin widths:
\begin{align}
...
%%%%%%%%%%  Basis
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{The basis functions} \label{sec:algorithm:basis}
